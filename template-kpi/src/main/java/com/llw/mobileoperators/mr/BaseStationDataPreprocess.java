...
public class BaseStationDataPreprocess {
	...
	public static class Map extends Mapper<LongWritable, Text, Text, Text>{
		...
		String date;	//记录日期点
		String[] timepoint;//记录读取分割点
		boolean dataSource;//判断文件是否以“POS”开头
			...
			while(it.hasNext()) {
				...
				if(diff <= 60.0) {
					if(locs.containsKey(upload.getValue())) {
						locs.put(upload.getValue(), 
								locs.get(upload.getValue()) + diff);
					} else {
	...
	public static void main(String[] args) throws ClassNotFoundException, IOException, InterruptedException {
		if(args.length != 4) {
			...
			System.err.println("\t"+"USERKIP"+"\t"+"users in some time are invalid");
			System.err.println("");
			System.exit(-1);
		}
		run(args);
